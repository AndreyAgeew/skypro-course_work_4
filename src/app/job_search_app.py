...
from src.api.zarplata_api import ZarplataAPI
...
class JobSearchApp(metaclass=JobSearchAppMeta):
    ...
    __zarplata_api: ZarplataAPI = ZarplataAPI()
    ...
    def __search_vacancies(cls) -> None:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            zp_future = executor.submit(cls.__zarplata_api.search_vacancies, cls.__job_title)
            hh_vacancies = hh_future.result()
            sj_vacancies = sj_future.result()
            zp_vacancies = zp_future.result()
            for vacancy_data in hh_vacancies + sj_vacancies + zp_vacancies:
